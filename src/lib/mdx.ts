...
import { compareTwoStrings } from '~/lib/dice-coefficient-kr';
  ...
  return postList
    ...
    .map((p) => {
      const tagPoint = post.data.tags
        ? post.data.tags.filter((tag: string) => p.data.tags?.includes(tag))
            .length
        : 0;
...
export const parseToc = (source: string) => {
  console.log('parseToc', source);

  return source
    .split('\n')
    .filter((line) => line.match(/(^#{1,3})\s/) && !line.startsWith('# '))
